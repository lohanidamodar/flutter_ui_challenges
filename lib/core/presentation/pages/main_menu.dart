...
class _MainMenuState extends State<MainMenu> {
  ...
  ListView _buildMenuPage() {
    return ListView(
      ...
      children: <Widget>[
        ...pages.map((page) => page is m.MenuItem
  ...
  Widget _buildExpandableMenu(m.MenuItem page, BuildContext context) {
  ...
  List<Widget> _buildSubMenus(List<m.SubMenuItem> items, BuildContext context) {
    final List<Widget> subMenus = [];
    items.forEach((item) => subMenus.add(_buildSubMenu(item, context)));
    return subMenus;
  }

  Widget _buildSubMenu(m.SubMenuItem item, BuildContext context) {
    return Padding(
      ...
      child: ListTile(
        ...
        trailing: Row(
          ...
          children: <Widget>[
            IconButton(
              ...
              onPressed: () => _openPage(context, item, m.OpenMode.CODE),
            ),
          ],
        ),
        title: Text(
        ...
        onTap: () => _openPage(context, item, m.OpenMode.PREVIEW),
      ),
    );
  }

  void _openPage(BuildContext context, m.SubMenuItem item, m.OpenMode mode) {
    Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => mode == m.OpenMode.CODE
              ? DesignPreviewsPage(
